...
void PerBlockFileProcessor::ProcessFile(const char* output_path) {
  ...
  for (std::size_t i = 0; i < complete_blocks_; ++i, input_region_ptr += block_size_bytes_) {
    auto processed_block = thread_pool_.EnqueueJob([this](const unsigned char* const block_ptr, std::size_t size) {
      return ProcessBlock(block_ptr, size);
    }, input_region_ptr, block_size_bytes_);
  ...
  if (trailing_block_exists_) {
    ...
    std::vector<unsigned char> result = ProcessBlock(input_region_ptr, remainder_size_bytes);
